...
export function makeEvmRpc({ s, chain, evm, carrier, log = () => {} }) {
  ...
  // a read-only call sees the block it would land in (next height, now): contracts that read block.timestamp or block.number must not see zeros
  const nextBlock = () => evm.lib.blk.createBlock({ header: { number: BigInt(s.height() + 1), timestamp: BigInt(Math.floor(Date.now() / 1000)), gasLimit: evm.gasLimit, coinbase: util.createZeroAddress(), baseFeePerGas: 1000000000n } }, { common: evm.common });
  const callOpts = (c) => ({ block: nextBlock(), to: c.to ? addr(c.to) : undefined, caller: addr(c.from ?? ZEROADDR), value: c.value ? BigInt(c.value) : 0n, data: c.data ?? c.input ? util.hexToBytes(c.data ?? c.input) : new Uint8Array(), gasLimit: c.gas ? BigInt(c.gas) : evm.gasLimit });
  ...
  const methods = {
    ...
    eth_estimateGas: async ([c]) => withCheckpoint(async () => { const r = await evm.vm.evm.runCall(callOpts(c)); if (r.execResult.exceptionError) { const e = new Error('execution reverted'); e.code = 3; e.data = hexb(r.execResult.returnValue); throw e; } const data = callOpts(c).data; const calldata = [...data].reduce((a, b) => a + (b === 0 ? 4 : 16), 0); const exec = r.execResult.executionGasUsed; return hexn(21000n + (c.to ? 0n : 32000n) + BigInt(calldata) + (exec > 0n ? exec * 3n / 2n + 10000n : 0n)); }), // a plain transfer is exactly 21000; a call under-counts what a transaction pays (cold accounts, value transfers, first writes), so half again plus a margin — unused gas is not charged

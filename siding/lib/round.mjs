...
export function makeRound({ engine, s, chain, fed, key, pub, relays, events, publish, subscribe, log = () => {}, proposeAfter = 30, onBlock = () => {}, checkClaims = null }) {
  ...
  let lastBlockAt = Date.now(), lastHeightSeen = s.height(); let claimsWanted = []; let dueSince = null; // when the next block became due: lateness counts from there, not from the last block
  ...
  async function propose(opts = {}) {
    ...
    log(`round: proposing h${height} ${ev.id.slice(0, 12)}… (${block.transactions.length - 1} txs, ${claims} claim(s))`); const okc = await send(ev); log(`round: proposal h${height} ${ev.id.slice(0, 12)}… reached ${okc} relay(s)`);
    await maybeSeal();
  }
  async function onProposal(ev) {
    const height = Number(tag(ev, 'h')); if (!Number.isInteger(height) || ev.pubkey === pub) return;
    if (Date.now() / 1000 - ev.created_at > proposeAfter * n) return; // a relay replaying an old proposal: its proposer has moved on
  ...
  return {
    ...
    async tick({ due }) {
      ...
      if (!due) { dueSince = null; return; } dueSince ??= Date.now(); const height = s.height() + 1; if (!mayReSign(height)) return;

...
throws('produce() is refused on a federated chain', () => s.produce(keys[0]), /through the round/);
const { block: b1 } = s.buildNext({ time: s.tip().time + 1 });
...
t('block 1 sealed by signers 1 and 2 is accepted', s.addSealed(sealed).height === 1);
const { block: b2 } = s.buildNext({ time: s.tip().time + 1 });
t('block 2 sealed by signers 2 and 3 (a different pair) is accepted', s.addSealed(sealWith(b2, [1, 2])).height === 2);
const { block: b3 } = s.buildNext({ time: s.tip().time + 1 });
throws('one signature is refused by the validator', () => { const sigs = new Map([[pubs[0], partialSignature(E, b3, fed, keys[0])]]); s.addSealed(sealBlock(E, b3, assembleWitness({ ...fed, threshold: 1 }, sigs))); });
throws('assembleWitness refuses fewer than k signatures', () => assembleWitness(fed, new Map([[pubs[0], p0]])), /1 of 2/);
throws('signatures in the wrong slots are refused', () => { const w = assembleWitness(fed, new Map([[pubs[0], partialSignature(E, b3, fed, keys[0])], [pubs[1], partialSignature(E, b3, fed, keys[1])]])); [w[1], w[2]] = [w[2], w[1]]; s.addSealed(sealBlock(E, b3, w)); });
throws('a signature over a different block is refused', () => { const sigs = new Map([[pubs[0], p0], [pubs[1], p1]]); s.addSealed(sealFederated(E, b3, fed, sigs)); });
t('and a correct third block after all that is accepted', s.addSealed(sealWith(b3, [0, 1])).height === 3);

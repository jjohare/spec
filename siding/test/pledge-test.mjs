...
const reward = { txid: 'ab'.repeat(32), vout: 0, value: 5000000000, script: mine, height: 151260 };
t('maturity: a reward from the locked era matures at the document height; an older one at +100', maturityOf(151260, chain.pledge) === 158111 && maturityOf(151000, chain.pledge) === 151100);
...
const v = verifyPledge({ k, hex: b.hex, chain, prevout, parentTip: 151270 });
...
t('refused: not a coinbase', /not a coinbase/.test(verifyPledge({ k, hex: b.hex, chain, prevout: { ...prevout, coinbase: false }, parentTip: 151270 }).error));
t('refused: spent or unknown output', /not an unspent/.test(verifyPledge({ k, hex: b.hex, chain, prevout: null, parentTip: 151270 }).error));
t('refused: a reward from before the lock', /not locked/.test(verifyPledge({ k, hex: b.hex, chain, prevout: { ...prevout, height: 151000 }, parentTip: 151270 }).error));
t('refused: already mature', /already mature/.test(verifyPledge({ k, hex: b.hex, chain, prevout, parentTip: 158111 }).error));
t('refused: wrong owner (signature)', /signature/.test(verifyPledge({ k, hex: b.hex, chain, prevout: { ...prevout, script: other }, parentTip: 151270 }).error));
...
t('refused: wrong lock time', /lock time/.test(verifyPledge({ k, hex: b2.hex, chain, prevout, parentTip: 151270 }).error));
t('refused: too large for the desk', /maximum/.test(verifyPledge({ k, hex: b.hex, chain: { ...chain, pledge: { ...chain.pledge, maxPerPledge: 1000 } }, prevout, parentTip: 151270 }).error));

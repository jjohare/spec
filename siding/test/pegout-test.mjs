...
await throws('submit refuses a burn below pegoutMin', async () => await s.submit(spend([{ value: 9999, scriptPubKey: pegoutMarker(PARENT) }, { value: coin.value - 9999 - 1000, scriptPubKey: me }]).hex), /at least 10000/);
await throws('submit refuses a pegout: marker that is not a script', async () => await s.submit(spend([{ value: 20000, scriptPubKey: '6a0a' + Buffer.from('pegout:zz').toString('hex') }, { value: coin.value - 21000, scriptPubKey: me }]).hex), /2 to 40 bytes/);
const burn = spend([{ value: 50000, scriptPubKey: pegoutMarker(PARENT) }, { value: coin.value - 50000 - 1000, scriptPubKey: me }]);
t('submit accepts a burn of 50000 sats', (await s.submit(burn.hex)).txid === burn.txid);
const before = s.utxo.size; const r = await s.produce(key); const after = [...s.utxo.values()].reduce((a, c) => a + c.output.value, 0);
...
await throws('validator: a burn in the coinbase is refused', async () => await s.addBlock(mk([{ value: 0, scriptPubKey: pegoutMarker(PARENT) }])));
t('and an ordinary block after that still produces', (await s.produce(key)).height === s.tip().height);

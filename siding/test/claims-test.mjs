...
t('an empty block produces', (await s.produce(key)).height === 1);
const you = '5120' + signer.pubkeyOf(signer.randomKey());
const r = await s.produce(key, { claims: [{ txid: PEG1, vout: 0, amount: 25e8, script: you }] });
...
await throws('the producer refuses to claim an outpoint twice', async () => await s.produce(key, { claims: [{ txid: PEG1, vout: 0, amount: 25e8, script: you }] }), /already claimed/);
...
await throws('validator: a duplicate claim from another block is refused', async () => await s.addBlock(mk([{ value: 25e8, scriptPubKey: you }, { value: 0, scriptPubKey: claimMarker(PEG1, 0) }])));
await throws('validator: coinbase value without a claim marker is refused', async () => await s.addBlock(mk([{ value: 1e8, scriptPubKey: you }])));
await throws('validator: a marker with no payout before it is refused', async () => await s.addBlock(mk([{ value: 0, scriptPubKey: claimMarker(PEG2, 0) }, { value: 1e8, scriptPubKey: you }])));
await throws('validator: two markers sharing one payout are refused', async () => await s.addBlock(mk([{ value: 1e8, scriptPubKey: you }, { value: 0, scriptPubKey: claimMarker(PEG2, 0) }, { value: 0, scriptPubKey: claimMarker(PEG2, 1) }])));
await throws('validator: the same outpoint twice in one block is refused', async () => await s.addBlock(mk([{ value: 1e8, scriptPubKey: you }, { value: 0, scriptPubKey: claimMarker(PEG2, 0) }, { value: 1e8, scriptPubKey: you }, { value: 0, scriptPubKey: claimMarker(PEG2, 0) }])));
t('a well-formed claim block from outside is accepted', (async () => { await s.addBlock(mk([{ value: 1e8, scriptPubKey: you }, { value: 0, scriptPubKey: claimMarker(PEG2, 0) }])); return s.claimed(PEG2, 0) && s.coins(you).length === 2; })());
t('and an ordinary block after all that still produces', (await s.produce(key)).height === s.tip().height);
...
while (s.tip().height < 101) await s.produce(key); // mature the genesis coin
...
await throws(`submit refuses a zero-fee transaction (${vs} vB needs ${vs} sats)`, async () => await s.submit(spend(0)), /below the minimum/);
await throws('submit refuses one sat short', async () => await s.submit(spend(vs - 1)), /below the minimum/);
t('submit accepts exactly the minimum', (async () => { const r = await s.submit(spend(vs)); return r.fee === vs && r.vsize === vs; })());

...
const throws = (name, fn, re) => { try { fn(); t(name + ' (did not throw)', false); } catch (e) { t(name + (re && !re.test(e.message) ? ` (threw: ${e.message.slice(0, 100)})` : ''), !re || re.test(e.message)); } };
...
t('submit accepts an issuance of 1,000,000 SHELL onto output 0', s.submit(issue.hex).txid === issue.txid);
s.produce(key); const SHELL = issue.txid;
...
throws('tally:self without issue: is refused', () => s.submit(mk([coin()], [{ value: 1000, scriptPubKey: me }, { value: coin().value - 3000, scriptPubKey: me }, { value: 0, scriptPubKey: recordScript('tally:self:0=5') }]).hex), /tally:self without/);
throws('assigning an asset the inputs do not carry is refused', () => s.submit(mk([coin()], [{ value: 1000, scriptPubKey: me }, { value: coin().value - 3000, scriptPubKey: me }, { value: 0, scriptPubKey: recordScript(`tally:${SHELL}:0=1`) }]).hex), /but carries 0/);
throws('a tally naming an OP_RETURN output is refused', () => s.submit(mk([coin()], [{ value: 1000, scriptPubKey: me }, { value: coin().value - 3000, scriptPubKey: me }, { value: 0, scriptPubKey: recordScript('issue:BAD:0') }, { value: 0, scriptPubKey: recordScript('tally:self:2=5') }]).hex), /an OP_RETURN/);
throws('a malformed record of ours is refused', () => s.submit(mk([coin()], [{ value: 1000, scriptPubKey: me }, { value: coin().value - 3000, scriptPubKey: me }, { value: 0, scriptPubKey: recordScript('issue:toolongticker:0') }]).hex), /malformed/);
...
t('a transfer of 600k + 300k (100k burned by omission) is accepted', s.submit(xfer.hex).txid === xfer.txid); s.produce(key);
...
t(`opening a pool with ${X0} sats + ${Y0} SHELL, ${S0} shares, is accepted`, s.submit(open.hex).txid === open.txid); s.produce(key);
...
throws(`taking ${out + 1} SHELL (one unit past the invariant) is refused`, () => s.submit(swapTx(out + 1).hex), /constant product/);
const sw = swapTx(out); t(`taking ${out} SHELL for ${dx} sats is accepted (pool input witness empty: OP_TRUE)`, s.submit(sw.hex).txid === sw.txid); s.produce(key);
...
throws(`minting ${mint + 1} shares for that add is refused`, () => s.submit(addTx(mint + 1).hex), /at most/);
const ad = addTx(mint); t(`adding ${ax} sats + ${ay} SHELL mints ${mint} shares`, s.submit(ad.hex).txid === ad.txid); s.produce(key);
...
throws('taking one sat more than pro rata is refused', () => s.submit(remTx(rx + 1).hex), /pro-rata/);
const rm = remTx(rx); t(`removing ${mint} shares takes ${rx} sats + ${ry} SHELL`, s.submit(rm.hex).txid === rm.txid); s.produce(key);
...
s.submit(s1.hex);
throws('a second swap of the same pool coin is refused by the mempool as a double spend', () => { const c = mature().filter((x) => x.outpoint !== s1.tx.inputs[0].prevout.txid + ':' + s1.tx.inputs[0].prevout.vout).sort((a, b) => b.value - a.value)[0]; s.submit(mk([c], [{ value: p3.x + 1, scriptPubKey: '51' }, { value: 1000, scriptPubKey: me }, { value: c.value - 1 - 1000 - 3000, scriptPubKey: me }, { value: 0, scriptPubKey: recordScript(`pool:${POOL}:0`) }, { value: 0, scriptPubKey: recordScript(`tally:${SHELL}:0=${p3.y}`) }], [p3.outpoint]).hex); }, /already spent in the mempool/);
const r = s.produce(key); t('the block with the first swap is produced', r.txs === 2 && P().outpoint === `${s1.txid}:0`);
...
throws('validator: a block whose transaction assigns what it does not carry is refused', () => s.addBlock(s.k.codec.encodeHex('Block', signBlock({ ...engine, interpreter: s.k.interpreter, schnorrSign: signer.schnorrSign }, bb, chain.challenge, key))));
t('and the chain still produces after the refusal', s.produce(key).height === s.tip().height);
